...
---@field api_key string? The API key for cloud endpoints (Bearer token)
...
local function get_auth_headers()
	if M.config.api_key and #M.config.api_key > 0 then
		return { Authorization = "Bearer " .. M.config.api_key }
	end
	return nil
end
...
local function parse_prompt(prompt)
    ...
    if original_text:find("$sel") then
        ...
        -- normalize: ensure sel_start is always before sel_end
...
function M.prompt(name)
	...
	if headers then
		curl_opts.headers = headers
	end

	local job = require("plenary.curl").post(M.config.url .. "/api/generate", curl_opts)
	---@param j Job
...
local function query_models()
	...
	local res = require("plenary.curl").get(M.config.url .. "/api/tags", curl_opts)
